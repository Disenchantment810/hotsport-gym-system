...
if(isset($_POST['submit'])) {
  ...
  if($email != "" && $plainPassword != "") {
    try {
      $query = "select id, fname, lname, email, mobile, password, address, create_date from tbluser where email=:email";
      ...
      if(!empty($row) && !empty($row['password']) && password_verify_compat($plainPassword, $row['password'])) {
        ...
        if (password_needs_upgrade($row['password'])) {
          $rehash = $dbh->prepare("update tbluser set password=:newpassword where id=:id");
